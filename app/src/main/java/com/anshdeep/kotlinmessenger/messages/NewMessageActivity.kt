    ...
    private fun fetchUsers() {
        ...
        ref.addListenerForSingleValueEvent(object : ValueEventListener {
            ...
            override fun onDataChange(dataSnapshot: DataSnapshot) {
                ...
                //Toast.makeText(this@NewMessageActivity, "2", Toast.LENGTH_SHORT).show()
                dataSnapshot.children.forEach {
                    Log.d(TAG, it.toString())
                    @Suppress("NestedLambdaShadowedImplicitParameter")
                    it.getValue(User::class.java)?.let {
                        if (it.uid != FirebaseAuth.getInstance().uid) {
                           // Toast.makeText(this@NewMessageActivity, "5", Toast.LENGTH_SHORT).show()
                            adapter.add(UserItem(it, this@NewMessageActivity))
                        }
                    }
                    //Toast.makeText(this@NewMessageActivity, "4", Toast.LENGTH_SHORT).show()
                }
